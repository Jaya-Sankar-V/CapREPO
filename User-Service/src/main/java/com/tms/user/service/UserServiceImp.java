...
public class UserServiceImp implements UserService {
	...
	{
		UserEntity userentity = usreRepository.findById(uid).orElseThrow(() -> new UserUidNotFoundException("UserID not Found"));
	...
	{
		UserEntity userentity = usreRepository.findById(id).orElseThrow(() -> new UserUidNotFoundException("User Id Not Found"));
		
		UserDTO userdto = modelMapper.map(userentity, UserDTO.class);
		
		ProjectDTO projectdto = projectClient.getProjectByPid(userentity.getProjectId());
	...
	@Override
	public List<UserDTO> getUserByProjectId(Long projectId) 
	{
		List<UserEntity> userentitylist = usreRepository.findAllByProjectId(projectId);
		return userentitylist.stream().map(entity -> modelMapper.map(entity, UserDTO.class)).collect(Collectors.toList());
